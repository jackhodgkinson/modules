# Module to setup, generate, load MedMNIST datasets
...
### User created modules - load from modules folder
from data_manip import dataset_namer

from file_path import file_path

## Create MedMNIST class 
...
def medmnist_generator(data_flag, size):
    ...
    #For loop over splits
    for i in splits:

        # Use of Dataset_Namer function to encode outputs
        ds_name = dataset_namer(data_flag, i, size)

        try:
            value = DataClass(split=i, size=int(size), transform=data_transform, download=True)
            dataset[ds_name] = value

        except Exception as e:
            print(f"Error in medmnist_generator: {e}")
    
    train = dataset[f"{data_flag}_train_{size}"]
    test = dataset[f"{data_flag}_test_{size}"]
    val = dataset[f"{data_flag}_val_{size}"]
    
    return train, test, val

## Function to load MedMNIST datasets
def medmnist_loader(dataset, resolution):

    # Create folder for results
    name = dataset_namer(dataset,resolution)
    path = os.path.join(file_path+'Results/', name)
    ...
    data = medmnist_generator(dataset, resolution)
